#!/bin/bash
#SBATCH --job-name=woo_wgs_aln_woodsorrel_v0.1_racon_x2_medaka_x2		# Job name 
...
FILE=polishing/woodsorrel/medaka_x2/woodsorrel_v0.1_racon_x2_medaka_x2.fasta.bwt
if [ -f "$FILE" ] && [ -s "$FILE" ]; then
    echo "$FILE exists and is not empty. Start Alignment."
else 
    echo "$FILE does not exist or is empty. Creating Index."
    bwa index polishing/woodsorrel/medaka_x2/woodsorrel_v0.1_racon_x2_medaka_x2.fasta
fi

#BWA MEM alignment
bwa mem -t 30 polishing/woodsorrel/medaka_x2/woodsorrel_v0.1_racon_x2_medaka_x2.fasta \
trimmed_reads/woo_aa_PE1_cutadapt.fq.gz \
trimmed_reads/woo_aa_PE2_cutadapt.fq.gz \
> bwa_mem_mapping/woo_aa_aln_to_woodsorrel_v0.1_racon_x2_medaka_x2.sam

#Mark duplicates with PICARD tools
java -Xmx100G -jar $EBROOTPICARD/picard.jar MarkDuplicates \
I=bwa_mem_mapping/woo_aa_aln_to_woodsorrel_v0.1_racon_x2_medaka_x2.sam \
O=bwa_mem_mapping/woo_aa_aln_to_woodsorrel_v0.1_racon_x2_medaka_x2.marked_dup.sam \
M=bwa_mem_mapping/woo_aa_aln_to_woodsorrel_v0.1_racon_x2_medaka_x2.marked_dup_metrics.txt 

samtools sort -@ 30 \
-o bwa_mem_mapping/woo_aa_aln_to_woodsorrel_v0.1_racon_x2_medaka_x2.marked_dup.sorted.bam \
bwa_mem_mapping/woo_aa_aln_to_woodsorrel_v0.1_racon_x2_medaka_x2.marked_dup.sam
...
samtools index -@ 30 bwa_mem_mapping/woo_aa_aln_to_woodsorrel_v0.1_racon_x2_medaka_x2.marked_dup.sorted.bam
...
#sbatch woo_wgs_aln_woodsorrel_v0.1_racon_x2_medaka_x2.sh
